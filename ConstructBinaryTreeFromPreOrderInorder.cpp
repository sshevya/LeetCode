/********** no need to find index of root pass by reference ******/
...
public:
    ...
    TreeNode* rec(vector<int>& preorder, vector<int> &inorder, int &i, int l, int r){
        ...
        cout<<in<<" ";
        root->left = rec(preorder, inorder, i, l, in-1);
        root->right = rec(preorder, inorder, i, in+1, r);
    ...
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        int n = preorder.size();
        int i =0;
        TreeNode* root = rec( preorder, inorder, i, 0, n-1);
        return root;
    }
};
...
/********* finding index of root and passing ******/
/**
 * Definition for a binary tree node.
 * struct TreeNode {
 *     int val;
